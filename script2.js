...
function generatePasswordRandomness(userInput, pLenght) {
  ...
  // Now that we finish and concatenaed our string user input criteria randomness, we move to finish with the rest of the p.lenght. Our example of 8 range is now 6 left or while  2 < 8.
  ...
  // final password is ready to be returned! --> finalPassword back to displayPassword
...
function writePassword() {
  ...
  arraySplit = ""; // Used to translate inpute array back to the user as a string with slices
  exitApplication = false; // on/off switch for running the app
  userInput = ""; // Important - will be used to generate password base on criteria
  finalPassword = ""; // Important- used as a variable to store temporarily radom password. 
  displayPassword = ""; //Important- used to display the final password on the text area of the HTML element.
  window.alert(" Lets check our passowrd criteria options");
    var password = generatePassword(); // Call the generatePassword(). Global variable used to instead for displayPassword.
  ...
  var passwordText = document.querySelector("#password"); // Part of the placeholder text querySelectors will be part of the later modules.
